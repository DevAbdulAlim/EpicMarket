<footer class="bg-gray-900 px-3 text-white py-12">
    <div class="max-w-7xl mx-auto">
        <div class="container mx-auto grid grid-cols-1  md:grid-cols-2 xl:grid-cols-4 gap-8">

            <!-- About Us -->
            <div class="mb-8 md:mb-0">
              <h2 class="text-3xl font-bold mb-4">About Us</h2>
              <p>Your ultimate source for trendy fashion and accessories. Explore the latest styles and express your unique self.</p>
            </div>

            <!-- Contact -->
            <div class="mb-8 md:mb-0">
              <h2 class="text-3xl font-bold mb-4">Contact Us</h2>
              <p>Email: info@example.com</p>
              <p>Phone: +123 456 7890</p>
            </div>

            <!-- Quick Links -->
            <div class="mb-8 md:mb-0">
              <h2 class="text-3xl font-bold mb-4">Quick Links</h2>
              <ul>
                <li><a href="#">Home</a></li>
                <li><a href="#">Shop</a></li>
                <li><a href="#">New Arrivals</a></li>
                <li><a href="#">Contact</a></li>
                <li><a href="#">Privacy Policy</a></li>
              </ul>
            </div>

            <!-- Follow Us -->
            <div class="flex flex-col">
              <h2 class="text-3xl font-bold mb-4">Follow Us</h2>
              <div class="flex space-x-4">
                <a href="#" class="text-white hover:text-gray-500">
                  <i class="fab fa-facebook-f text-2xl"></i>
                </a>
                <a href="#" class="text-white hover:text-gray-500">
                  <i class="fab fa-twitter text-2xl"></i>
                </a>
                <a href="#" class="text-white hover:text-gray-500">
                  <i class="fab fa-instagram text-2xl"></i>
                </a>
                <a href="#" class="text-white hover:text-gray-500">
                  <i class="fab fa-pinterest text-2xl"></i>
                </a>
              </div>
            </div>

          </div>

          <!-- Copyright -->
          <div class="mt-8 text-center text-gray-500">
            <p>&copy; 2023 EpicMarket. All rights reserved.</p>
          </div>
    </div>

  </footer>
