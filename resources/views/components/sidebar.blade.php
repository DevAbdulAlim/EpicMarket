...
<div x-data="{ isOpen: {{ $isOpen ? 'true' : 'false' }}, $isCompact }" :class="{ '-translate-x-full md:translate-x-0': !isOpen, 'translate-x-0': isOpen }"
    ...
    <div class="h-full w-screen md:w-full" x-data="{ activeDropdown: null }">
        ...
        <div class="h-full overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200 bg-blue-800 text-white flex-1 w-64 transition-all duration-500 ease-in-out"
            :class="{ 'w-12': isCompact, 'w-64': !isCompact }">

            <!-- Sidebar Header -->
            <div class="sticky top-0 flex items-center bg-blue-800 z-10 border-b justify-between">
            ...
            <nav class="flex flex-col mt-4 space-y-2 mb-4 z-10 bg-blue-800">
                @foreach ($navItems as $item)
                    <!-- If it has no dropdown -->
                    @if (!$item['dropdown'])
                        <div class="z-10 bg-blue-800 p-2"> <a href="{{ $item['route'] }}"
                                class="flex items-center p-2 rounded-md hover:border-l-2 border-primary hover:bg-blue-700 text-white bg-blue-800/10">
                                <i class="{{ $item['icon'] }}"></i>
                    ...
                    @else
                        <!-- Dropdown Section -->
                        {{-- compact --}}
                        <div class="z-10 p-2" :class="{ 'hidden': !isCompact }"
                            ...
                            <div class="flex items-center p-2 rounded-md hover:border-l-2 border-primary text-white cursor-pointer"
                                :class="{ 'bg-blue-700': activeDropdown === '{{ strtolower($item['name']) }}' }">
                                <a href="{{ $item['route'] }}">
                            ...
                            <ul x-show="activeDropdown === '{{ strtolower($item['name']) }}'" x-collapse
                                class="ml-8 z-10"
                                :class="{ 'absolute left-2 z-50 rounded-md overflow-hidden bg-white w-48 shadow-2xl -mt-10': isCompact }">
                                @foreach ($item['items'] as $subItem)
                                    <li :class="{ ' text-blue-800 hover:bg-blue-500 hover:text-white': isCompact }">
                                        <a class="block px-4 py-2" href="{{ $subItem['route'] }}" @click.stop>
                                            {{ $subItem['name'] }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="z-10 p-2" :class="{ 'hidden': isCompact }">
                            ...
                            <div @click="activeDropdown === '{{ strtolower($item['name']) }}' ? activeDropdown = null : activeDropdown = '{{ strtolower($item['name']) }}'"
                                :class="{
                                    'flex items-center p-2 hover:border-l-2 border-primary hover:bg-blue-700 rounded-md text-white cursor-pointer': true,
                                    'bg-blue-700': activeDropdown === '{{ strtolower($item['name']) }}'
                                }">
                            ...
                            <!-- Dropdown Items -->
                            <ul x-show="activeDropdown === '{{ strtolower($item['name']) }}'" x-collapse
                                class="ml-7 z-10 space-y-1 bg-blue-800 border-l"
                                :class="{ 'absolute z-50 bg-white w-fit shadow-2xl': isCompact }">
                                @foreach ($item['items'] as $subItem)
                                    <li class="hover:bg-blue-500">
                                        <a href="{{ $subItem['route'] }}" class="block p-2">
                                            {{ $subItem['name'] }}
                                        </a>
                                    </li>
                                @endforeach
